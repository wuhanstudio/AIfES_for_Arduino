/*
  AIfES CNN Training F32 Example (RT-Thread Version)
*/
...
static void aifes_cnn_training_f32_entry(void *parameter)
{
    uint32_t i;
    ...
    printf(
        "Required parameter memory: %lu bytes\n",
    ...
    printf(
        "Required training memory: %lu bytes\n",
        working_memory_size);
...
static void aifes_cnn_training_f32(int argc,char *argv[])
{
  tid1 = rt_thread_create("tu8g22",
                          aifes_cnn_training_f32_entry, RT_NULL,
                          THREAD_STACK_SIZE,
...
MSH_CMD_EXPORT(aifes_cnn_training_f32, AIfES CNN training demo (F32));